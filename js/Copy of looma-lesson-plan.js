...
// when search button is clicked - submit the 'search' form to looma-database.search.php
            $('#search').submit(function( event ) {
                  ...
                  if (!isFilterSet()) $("#innerResultsDiv").html("Please select at least 1 filter option before searching.")
                  else {
                  $.post( "looma-database-search.php",
                           $( "#search" ).serialize(),
                           function (result) {
...
function displayResults(results) {
      ...
      var result_array = [];
      result_array['dictionary'] = [];  //not searching for dictionary entries
      result_array['textbooks']  = [];  //not searching for textbooks
      result_array['chapters']   = [];  //need to add search for chapters
      result_array['activities'] = results;
...
//***********************
	...
	var collectionTitle = document.createElement("h1");
    	...
    	if (arraylength == 0) {
    	    collectionTitle.innerHTML = "No matching activities found";
        }
	...
	for(var i=0; i<filterdata_object.activities.length; i++) {
		var rElement = createActivityDiv(filterdata_object.activities[i], filterdata_object.activities[i-1]);  //BUG: array[i-1] not defined when i==0
            ...
            /*		CODE CHANGE: the commented code below appends a result DIV INSIDE another result DIV (if it is in the same "section"
             *                  this caused the nested result to be badly formatted. removed the logic below and just using the appendChild() above
             *
                if ($(rElement).data("type") == "chapter") {
            			actResultDiv.appendChild(rElement);
            		}
...
/*
    function extractItemId(ch_id) {
        ...
        var pieces = ch_id.match(/^([1-8])(M|N|S|SS|EN)([0-9][0-9])(\.[0-9][0-9])?$/);

        elements['currentGradeNumber'] = pieces[1];
        elements['currentSubject']     = pieces[2];
        elements['currentSection']     = pieces[3] ? pieces[2] : null;
        elements['currentChapter']     = pieces[3] ? pieces[3] : pieces[2];
...
var innerActivityDiv = function(item) {
                ...
                */
                ...
                $("<p/>", {
                    class : "result_dn",
                    html : "<b>" + item.dn.substring(0, 20) + "</b>"
                }).appendTo(textdiv);
