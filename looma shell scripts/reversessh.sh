...
# copy of the "reversessh" script on Termilus SSH Server (AWS)
#     this script registers a new remote system and generates the setup file for it as "ReverseSSH-<name>-Client.sh
...
rm ~/Desktop/r0* 2> /dev/null
touch ~/Desktop/$client

echo "Created /var/www/html/info with this contents:"
cat /var/www/html/info
echo
echo "Latest code update:    \$(cat /var/www/html/Looma/archivetimestamp.txt)"
echo "Latest content update: \$(cat /var/www/html/content/archivetimestamp.txt)"
EOT

  chmod 755  ReverseSSH-$client-Client.sh
