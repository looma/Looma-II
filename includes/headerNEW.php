...
// NOTE: trying to invoke "Cache-Control" setting "no-cache" using abpve PHP "header" command, and
//       also above HTML "meta" commands. not clear if it is working, or which of these commands is needed
...
//echo $ENV_IP;
    ...
    } else if ($ENV_IP = '127.0.0.1'){
        $LOOMA_SERVER = 'looma-local';
        error_reporting(E_ALL);
    } else {
        $LOOMA_SERVER = 'looma';
        error_reporting(E_ERROR); //note: change this on looma box when debugging
    }
...
/*
//DEBUG
echo '$LOOMA_SERVER is ' . $LOOMA_SERVER . '<br>';
echo '$ENV_OS is ' . $ENV_OS . '<br>';

if ($ENV_WINDOWS) echo '$ENV_WINDOWS is true<br>';
else echo '$ENV_WINDOWS is false<br>';

echo '$ENV_CPU is ' . $ENV_CPU . '<br>';
echo '$ENV_IP is ' . $ENV_IP . '<br>';
//DEBUG */
...
echo '<meta http-equiv="pragma" content="no-cache">';
    ...
    if ($LOOMA_SERVER === 'CEHRD') {
       echo "<title>Learning Portal</title>";
       echo '<link rel="icon" type="image/png" href="images/logos/CEHRD-logo small.jpg">';
    } else {  //server default is  'looma'
        echo "<title>{$page_title}</title>";
        echo '<link rel="icon" type="image/png" href="images/logos/looma favicon yellow on blue.png">';
    }
