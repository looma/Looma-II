...
///// LOOKUP /////
 *				 returns a JSON object {en:"word", np:"nepaliword",
 * 										defn:"definition", img: "filename of picture"}
 *				other properties (phonetic, useinsentence, partofspeech, hom, ant, syn) to be added later
///// ADD  //////
 * 				takes a JSON object {en:"word", np:"nepaliword", defn:"definition",
 * 									 img: "filename of picture"}
 *				validates properties and inserts the word in the database
 *				and returns success: TRUE or FALSE
///// LIST /////
 * 				takes an object {class, subject, [ch_id], [count], [random:boolean]}
 *               and returns an array of english words
 *				matching the filter criteria, length=count
 * 				and randomized if random=true
///// LISTWithProperties /////
 * 				takes an object {class, subject, [ch_id], [count], [boolean]}
 * 				and returns an array of
 * 				objects {en, np, rw, plural... <all the properties in the dictionary}
 *				matching the filter criteria, length=count
 * 				and randomized if boolean=true
 **/

include ('includes/mongo-connect.php');

//compatibility shiv for php 5.x "keyIsSet()"
function keyIsSet($key, $array) { return isset($array[$key]);}
...
function getWords() {
    ...
    } else if ($hasClass) {  //Handles providing words if only a class is provided
        $hasChapterId = true;
        $class = $_GET["class"];
        $startChapterId = substr($class, 5);
    } else if ($hasSubject) {  //Handles providing words if only a subject is provided
        $hasChapterId = true;
        $startChapterId = "[1-8]";
        $subject = $_GET["subject"];
        if ($subject == "english") $startChapterId .= "EN";
    ...
    } else {  //Handles providing words if neither a class or subject is provided
        $startChapterId = "";
        $hasChapterId = false;
    }

    //echo "startChapterId is " . $startChapterId;
...
//echo "found " . sizeof($words) . " words";
        ...
        // return the list in the order they appear in the database
        if ($hasChapterId) {
            ...
            $query = array();}
        $return = mongoFind($dictionary_collection, $query, null, null, 100*$maxCount);
    } //end not random
...
// accepted CMDs are 'lookup', 'add', 'list'
    ...
    switch ($cmd) {
        ...
        case "list":
            ...
            $list = array();
            foreach ($words as $newWord)
                array_push($list, $newWord['en']); //just return the english words
        ...
        case "listWithProperties":
            // given "class", "subj" OR "ch_id" and [optionally] "count" and "random" (boolean),
            // return an array of 'count' dictionary entry objects that match class&subj or ch_id
            $words = getWords();
            $list = array();
            foreach ($words as $newWord)
                array_push($list, $newWord); //return the full dictionary entry
        ...
        case "lookup":
            ...
            if (isset($_GET["word"]) && $_GET["word"] != "") {
                ...
                if ($word != null) {   //Add fields with blanks to avoid errors on code that receives words
