...
/*
    ...
    The zvec stack — the search service plus looma-ai — is what powers three
    ...
    The installer sets LOOMA_ZVEC=0 in that case (SetEnv in the Apache vhost for
    a native install, the service environment for Docker). Anything else — unset
...
if (!function_exists('looma_feature_flags_json')) {
    ...
    {
        $zvec = looma_zvec_enabled();
        $ai = looma_ai_enabled();
        return json_encode([
            'zvec' => $zvec,
            'search' => $zvec,        // looma-search
