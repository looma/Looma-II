...
/* LOOMA.speak()
 * Author: Akshay Srivatsan and Luke Arnold
 ...
 * also requires the wav file remover to be at "/Looma/looma-remove-file.php"
 */
    ...
    ////////////////////////////////////////////////////////////////////////////////////
        ...
        if (engine == 'synthesis') {
            ...
            if (LOOMA.speak.playingAudio != null) {
                ...
                var promiseList = []; //List of audio promises that >> audio files
                ...
                for (var i = 0; i < splitSentences.length; i++) {
                    ...
                    var audioPromise = new Promise (function(resolve, reject) {
                        var xhttp = new XMLHttpRequest(); //this will be an ajax request to looma-mimic.php to create the speech audio file
                        xhttp.onreadystatechange = function() {
                            if (this.readyState == 4 && this.status == 200) {//if/once audio creation succeeds
                                var fileLocation = xhttp.responseText;
                                currentAudio = new Audio(fileLocation);
                                console.log("loaded " + fileLocation);
                                
                                //this 'onended' handler is attached to each phrase before it is entered into the queue
                                currentAudio.onended = function () {
                                    console.log("End of Phrase: " + fileLocation);

                                    var remove = new XMLHttpRequest(); //this will be an ajax call to looma-remove-file.php to remove the audio file, as the currentAudio variable containing the speech has already been created
                                    remove.onreadystatechange = function() {
                                       if (this.readyState == 4 && this.status == 200) {
                                           console.log("Audio file removed: " + fileLocation);
                                       }
                                    }
                                    remove.open("GET", "looma-remove-file.php?file=" + fileLocation, true);
                                    remove.send(); //<^asynchronously removes speech file that was just played
                                    var nextPromise = promiseList.pop(); //takes the next audio promise off of the array
                                    if (nextPromise != null) {
                                        nextPromise.then(function(nextAudio){ //runs once the audio is finished loading and promise is fulfilled
                                        ...
                                        // There's nothing else to do, just cleanup
                                        console.log("Done with all phrases.");
                                        LOOMA.speak.cleanup();
                                    }
                                };//end currentaudio endfunc
                                resolve(currentAudio);//fulfill audio promise with currentAudio HTMLAudioElement object
                            }//end if(success)
                            else if (this.readyState == 4 && this.status == 404) {
                                reject(new Error("It broke"));
                            }//end if(fail)
                        };//end onreadystatechange{}
                        xhttp.open("GET", audioSource, true);
                        xhttp.send();
                    });//end promise def
                    promiseList.unshift(audioPromise);//list of promises, each item can be executed once it's done
                    console.log("promise created");
                            
                }  // end FOR loop which builds the queue of audio phrases to play
                
        
                var firstPromise = promiseList.pop();
                console.log("First promise recieved");
                firstPromise.then(function(firstAudio) {   
                    ...
                    if (playPromise !== undefined) {
                        ...
                        console.log("Javascript promise functionality is not supported by this browser");
                    };

                }); //end of firstpromise.then

                LOOMA.speak.activate(); //makes the speech button bigger
               
            }//end of else that calls looma-speech
