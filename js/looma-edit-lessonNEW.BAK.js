...
Date: version 1:spring 2016, version 2: Nov 16, version 3: spring 2018, version 4: JUL 20234
 */
...
function signature(elem) { //param is jQ object of the timeline ($timeline)
    ...
    elem.find('.activityDiv').each(function(index, x) {
        if ($(x).data('type') === 'inline') sig += $(x).attr('data-html');
        else                                sig += $(x).data('id');
    });
    return sig;
}
...
function lessonunpack (lesson) {  //unpack the array of collection/id pairs into html to display on the timeline
    ...
    $.when.apply(null, posts).then(function(){
        ...
        current_item = $('#timeline .activityDiv')[0];  // select first timeline item
        scroll_to_item(current_item);
...
function lessondisplay (lesson) {
    //clearFilter();
    $timeline.html(lessonunpack(lesson));
...
function preview_result (item) {
    ...
    // BUG?? - if previewing a search result this is not corrrect
    current_item = item;
...
function makesortable (){
    ...
    $("#timelineDisplay").sortable({
        ...
        //helper: "clone",
        update: function( event, ui ) {reindexTimeline();},
