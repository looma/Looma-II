            ...
            foreach($doc['events'] as $event) {
                  ...
                  $id2 = "";
                   ...
                   if(isset($event['popup'][0]))
                        $msg = 'data-msg=' .  bilingual($event['popup'][0],$event['npopup'][0] );

                  if(isset($event['popup'][1]))
                        $id1 = 'data-id1=' . bilingual($event['popup'][1],$event['npopup'][1]);

                  if(isset($event['popup'][2]))
                        $id2 = 'data-id2='. bilingual($event['popup'][2],$event['npopup'][2]);
                  ...
                  if ($count%2 == 0)
                  {
                   ...
                   echo '<button class="dropbtn"' .  " " . $id1 . " " . $id2 . " " . $msg . '>' .  bilingual($event['title'],$event['ndn']) . '</button>';

                   echo '<button class="dropdate">' . bilingual($event['date'],$event['ndate']) . '</button>'; //dropbtn before dropdate so dropbtn is on top
                  ...
                  echo '
                   <li>
                     ...
                     <div class="timeline-description">
                       ...
                       echo '<button class="dropdate">' . bilingual($event['date'],$event['ndate']) . '</button>';
                       echo '<button class="dropbtn"' . " " . $id1 . " " . $id2 . " " . $msg . '>' . bilingual($event['title'],$event['ndn']) . '</button>';
