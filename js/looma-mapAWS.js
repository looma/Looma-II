...
function loadBaseLayers (layerData) {
    ...
    for (var i = 0; i < layerData.length; i++)     // Loads the base layers onto the map by reading geojson in
    {
        ...
        promises[i] = $.getJSON(link, function (result)
        {
            baseLayer = L.geoJson(result, {
                style: styleLayer,
                onEachFeature: onEachFeature
            });
            baseLayers[nextBase] = baseLayer;
           // arrayIndex ++;
            nextBase ++;
            //currentBase = currentBase % baseLayers.length; //Will never go over, fixes a glitch
            //baseLayer.addTo(map).bringToBack();  //RACE condition - first layer to render will be shown 'on top', all others 'toBack'
            ...
            function lookUpColor (feature) {
                return feature.properties.fillColor || "red";
            }  //end lookUpColor
            ...
            function getColor(feature, style) {
                try {
                    ...
                    else return (lookUpColor(feature));  //custom lookup color for Nepal provinces and districts
        ...
        $.when.apply($, promises).then(function(){for (var layer of baseLayers) layer.addTo(map);baseLayers[0].bringToFront()});
    }
} // end loadBaseLayers()
    ...
    $.post("looma-database-utilities.php",
        ...
        function(data) {
            ...
            if (data.addOnLayers)
            {
                ...
                popUpsOn = new Array(data.addOnLayers.length);
            ...
            // Creates the checkboxes to toggle add-on layers on and off if add-on layers exist
            if (data.addOnLayers) addOnButtons(data.addOnLayers);
            
            
            // Creates the buttons to toggle between alternate base layers if there are multiple bases
            if (data.baseLayers && data.baseLayers.length > 1) baseLayerButtons(data.baseLayers);
