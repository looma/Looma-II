...
function play ($item) {
    ...
    playPDF($('#pdf')[0], $('#pdf').data('fn'),
            ...
            var length;
            if ( ft === 'chapter') {
                if (chapter_language === 'native' || lang === 'np') {  //(used in lesson-present: if language=='native' then show NP chapter if available
                    pagenumber = 1;
                    //filename = nfn;
                    filename = LOOMA.ch_idName(ch,'np');

                    //filepath = fp;
                    filepath = LOOMA.ch_idFilepath(ch,'np');

                    length = nlen || 100;
                } else {
                    filename = LOOMA.ch_idName(ch,'en');
                    filepath = LOOMA.ch_idFilepath(ch,'en');
                    pagenumber = 1;
                    length = len || 100;
                }
            } else length = 0;
            ...
            var previewSrc = filepath + filename + '#zoom=150#page=1' + '\"  style=\"height:100vh;width:100vw;\" type=\"application/pdf\"';
...
window.onload = function() {
   // $currentItem = null;
    ...
    $('#timeline-container').keydown(function(e) {
        if (e.which === 37) {
            e.preventDefault();
