...
function setFields(word) {
    ...
    if (word.def === "Word not found") {
        ...
        var i = 1;
        {
            var newRow = $('<tr class="row" id="row_"></tr>');
            ...
            $("#row-rw").val(word['rw']);
            $("#row-ch_id").val(word['ch_id']);  //change to show multiple ch_ids
            
            if ('def' in word && word['def']) $("#row-definition").val(word['def']).autoResize();
            else $("#row-definition").val(word['meanings'][0]['def']).autoResize();
            
            $("#row-id").val(word['_id']['$oid'] || word['_id']['$id'] || "newentry").css("display", "none");
...
function getDefinition(event) {
    ...
    LOOMA.lookup(word, setFields, fail);
    return false;
}
