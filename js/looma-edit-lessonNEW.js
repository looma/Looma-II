...
function signature(elem) { //param is jQ object of the timeline ($timeline)
    ...
    elem.find('.activityDiv').each(function(index, x) {
        if ($(x).data('type') === 'inline') sig += $(x).data('html');
        else                                sig += $(x).data('id');
    });
    return sig;
}
    ...
    if (collection == "chapters") {
        ...
        // Pictures
           ...
           // id = $(item).data('mongo').mongoID.$id || $(item).data('mongo').mongoID.$oid;
            ...
            //console.log($('#previewpanel') ? "OK" : "not OK");
            
            $('#previewpanel').empty().append($('<div class="textpreview text-display"></div>').html($(item).data('html')));
    
            $('.textpreview').attr('contenteditable', 'true');
    
            //document.querySelector("div#previewpanel").innerHTML = $(item).data('html');
...
function insertTimelineElement(source, target) {
    ...
    if ($dest.data('type') === 'text-template' || $dest.data('type') === 'text') cloneTextfile($dest, target,false);
    else
        {
 ...
 //       newname = currentname.match(LOOMA.CH_IDregex)[0] + " " + newname;
       ...
       $.post("looma-database-utilities.php",
        ...
        'json'
           ...
           //$( $clone + ' .textdiv .result_dn').text('inline');
           ...
           if ($clone.find('.result_ft').text().trim() === 'text-template') $clone.find('.result_ft').text('inline');
...
/*
$("#left").click(function () {
    $("div").css("text-align","left"); //need to find containing div?? add civ class="textline"??
});
*/

/*   &&&&&&&&&&&&&&&&&&&&&&
to remove multiple classes: (example with classname === regex(/color)
            ...
            Usage:  $(".myClass").removeClassStartingWith('color');
...
///////////////////////////////
    ...
    // ADD
    $('#innerResultsDiv'           ).on('mouseDown', '.add',        function() {
