...
# server, and without it their logs are indistinguishable. Empty = use the box's
# hostname. The IP is not a setting: `up` resolves it fresh on every boot.
BOX_NAME="${BOX_NAME:-}"
...
<VirtualHost *:80>
    ...
    # This box's name — the form's "Box name" field, else its hostname (see
    # box_name_label()). Stamped onto every trace/log the PHP shim emits
    # (includes/otel.php) as looma.device_name, same as Vector already does for
    # logs/metrics, so a shared OpenSearch/Grafana can tell boxes apart.
    SetEnv LOOMA_BOX_NAME @BOX_NAME@
    # This box's public IP at install time (empty if it had no internet then).
    # Stamped alongside looma.device_name so the data server's geoip ingest
...
box_name_label() {
  if [ -n "$BOX_NAME" ]; then echo "$BOX_NAME"
  else echo "$(hostname) (this box's hostname)"; fi
}
...
run_form() {
  ...
  while :; do
    ...
    local rows=(
      ...
      "boxname" "Box name (obs tag) .... $(box_name_label)"
...
Turn zvec on now? It brings semantic search, the AI Assistant and exam generation." \
      ...
      boxname)  ui_input BOX_NAME "Name that identifies this box in the observability data (Vector's box_name tag on logs/metrics, looma.device_name on app traces).

Blank = use the box's hostname. The box's LAN IP is added automatically as box_ip." "$BOX_NAME" || true ;;
...
install_deploy_native() {
  ...
  # This box's identity for the observability data — the form's "Box name"
...
cmd_up() {
  ...
  # apart). NAME is stable — the env file, else this box's hostname. IP is
  # resolved HERE, every boot, because DHCP can move it: `ip route get` gives the
  # address this box actually sends from; `hostname -I` is the fallback.
  LOOMA_BOX_NAME="${LOOMA_BOX_NAME:-$(hostname)}"
  LOOMA_BOX_IP="$(ip -4 route get 1.1.1.1 2>/dev/null | sed -n 's/.*[[:space:]]src[[:space:]]\{1,\}\([0-9.]\{1,\}\).*/\1/p' | head -n1)"
  [ -n "$LOOMA_BOX_IP" ] || LOOMA_BOX_IP="$(hostname -I 2>/dev/null | awk '{print $1}')"
  [ -n "$LOOMA_BOX_IP" ] || LOOMA_BOX_IP="unknown"
  # Public IP: captured once at install time (see cmd_install), NOT re-resolved
  # here — unlike box_ip this isn't about DHCP drift, and a network round-trip
  # on every single boot would fight the "app starts first, offline is fine"
