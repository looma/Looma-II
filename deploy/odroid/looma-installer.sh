...
Install flags (all optional; passing any flag skips the form):
  ...
  --box-name NAME         Name that identifies this box in the observability data
                          (Vector's \`box_name\` tag; its LAN IP is added as \`box_ip\`).
...
Turn zvec on now? It brings semantic search, the AI Assistant and exam generation." \
      ...
      boxname)  ui_input BOX_NAME "Name that identifies this box in the observability data (Vector's box_name tag).
...
# Name Vector stamps on every log/metric from this box as \`box_name\` (\`box_ip\`
# is added at 'up' time). Change it and re-run 'looma-installer.sh up'.
