...
if ($tb_fn != null) {
    echo "<button class='en-chapter heading img' id='englishTitle' disabled>" .
        ...
        //$grade . " " . $caps .
        "<div>Chapter Lessons</div>" .
        "<img src=" . $tb_fp . thumbnail($tb_fn) . "></button>";
    echo "<button class='en-lesson heading img activities' disabled>"; echo "Textbook"; echo "</button>";
...
if ($tb_nfn != null) {
    ...
    echo "<button class='np-lesson heading img activities' disabled>"; echo "पाठ्यपुस्तक"; echo "</button>";
...
// display a button for the lesson plans for this chapter
        ...
        if ($lesson) {
            echo "<button class='lesson en-chapter'
                      data-lang='en'
                      data-ch='$ch_id'
                      data-len='$ch_len'
                      data-chdn='" .
...
// display chapter button for english textbook, if any
    if ($tb_fn && $ch_pn) { echo "<button class='$ch_ft en-lesson' 
                                      data-lang='en' 
                                      data-fn='$tb_fn' 
                                      data-fp='$tb_fp' 
                                      data-ch='$ch_id'  
...
///     //check in the database to see if there are any LESSON PLANS for this CHAPTER. if so, create a button
        ...
        $lesson = mongoFindOne($activities_collection, $query);
        if ($lesson) {
            echo "<button class='lesson np-chapter'
                          data-lang='np'
                         data-ch='$ch_id'
                           data-chdn='" .
                $lesson['dn'] .
...
////////// NEPALI chapter ///////////
    ...
    if ($tb_nfn && $ch_npn) { echo "<button class='$ch_ft np-lesson'
                                    data-lang='np'
                                    data-fn='$tb_nfn'
                                    data-fp='$tb_fp'
                                    data-ft='$ch_ft' 
